...
class GetFechamentoRelatorioUseCase {
    ...
    private styleColumnsProduto(workbook: any, sheetName: string) {
        workbook.Sheets[sheetName]["!cols"] = [{ wpx: 50 }, { wpx: 200 }, { wpx: 200 }, { wpx: 200 }, { wpx: 200 }]
    }
    ...
    private async addDataToSheet(workbook: any, sheetName: string, data: any) {
        const dataForExcel = []
        const entradas = data.detalhes.entradas.reduce((acc, item) => acc + parseFloat(item.valor), 0).toFixed(2)
        const despesas = data.detalhes.despesas.reduce((acc, item) => acc + parseFloat(item.valor), 0).toFixed(2)
        const fechamento = parseFloat(data.caixa) - parseFloat(data.controle_despesas)

        dataForExcel.push(["", "", "", ""])

        dataForExcel.push([
            `Total Entradas: R$ ${entradas}`,
    ...
    private async addDataToSheetProduto(workbook: any, sheetName: string, data: any) {
        ...
        dataForExcel.push([
            ...
            "",
            "",
            `Valor Total: R$ ${data.reduce((acc, item) => acc + parseFloat(item.valor), 0).toFixed(2)}`,
            "Quantidade Total: " + data.reduce((acc, item) => acc + item.quantidade, 0),
        ])
        dataForExcel.push(["", "", "", ""])

        data.forEach((item) => {
            dataForExcel.push([
                item.sequencial.toString(),
                item.data_pedido.toLocaleDateString("pt-BR"),
                `R$ ${parseFloat(item.valor).toFixed(2)}`,
