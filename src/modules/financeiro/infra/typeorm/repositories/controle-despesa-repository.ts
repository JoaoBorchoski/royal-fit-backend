  ...
  async list(search: string, page: number, rowsPerPage: number, order: string, filter: string): Promise<HttpResponse> {
    ...
    try {
      ...
      const controleDespesas = await query
        .andWhere(
          new Brackets((query) => {
            query.andWhere("CAST(cast(con.dataVencimento as VARCHAR) AS VARCHAR) ilike :search", { search: `%${search}%` })
          })
        )
        .addOrderBy("con.dataVencimento", columnOrder[0])
        .addOrderBy("con.valor", columnOrder[1])
        .addOrderBy("con.status", columnOrder[2])
        .offset(offset)
        .limit(rowsPerPage)
        .take(rowsPerPage)
        .getRawMany()
