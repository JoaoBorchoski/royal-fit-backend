...
class RelatorioFuncionarioRepository implements IRelatorioFuncionarioRepository {
  ...
  // create
  async create({ funcionarioId, dataInicio, dataFim, relatório, desabilitado }: IRelatorioFuncionarioDTO): Promise<HttpResponse> {
  ...
  async list(search: string, page: number, rowsPerPage: number, order: string, filter: string): Promise<HttpResponse> {
    ...
    const referenceArray = ["funcionarioNome", "dataInicio", "dataFim"]
  ...
  async select(filter: string): Promise<HttpResponse> {
    try {
      const relatoriosFuncionarios = await this.repository
        .createQueryBuilder("rel")
        .select(['rel. as "value"', 'rel. as "label"'])
  ...
  async idSelect(id: string): Promise<HttpResponse> {
    try {
      const relatorioFuncionario = await this.repository
        .createQueryBuilder("rel")
        .select(['rel. as "value"', 'rel. as "label"'])
        .where("rel. = :id", { id: `${id}` })
        .getRawOne()
  ...
  async count(search: string, filter: string): Promise<HttpResponse> {
    try {
      let query = this.repository.createQueryBuilder("rel").select(['rel.id as "id"']).leftJoin("rel.funcionarioId", "a")
  ...
  async getPedidosByData(dataInicio: Date, dataFim: Date): Promise<HttpResponse> {
    try {
      ...
      const newPedidos = destructor({
        data: pedidos,
        ref: "funcionarioId",
        variablesToArray: ["id", "data", "valorTotal", "desconto", "desabilitado", "funcionarioId", "funcionarioNome"],
